...
* Bundle `uv` binary + pyproject.toml + lockfile; on first launch, `uv sync` installs Python + all deps from PyPI (including `mng` and `imbue-minds`)
* Bundle a platform-appropriate git binary so there are no external prerequisites -- the app is fully self-contained
* Publish `imbue-minds` (the minds app) to PyPI so it can be installed via `uv` just like `mng` -- no source bundling needed
...
* The bundled `uv` binary handles Python environment setup. A platform-appropriate `git` binary is included. All Python packages (including `mng` and `imbue-minds`) are installed from PyPI via `uv sync` -- no source code is bundled, only pyproject.toml + lockfile.
...
* All backend logs are written to a log file in the app's data directory, accessible via a "View Logs" menu item (use similar logging conventions to how mng already writes logs)
...
* `electron/pyproject.toml` -- Standalone pyproject.toml that declares `imbue-minds` (and transitively `mng`) as a dependency, with a lockfile pinning exact versions. This is what `uv sync` operates on inside the app -- it is separate from the monorepo's pyproject.toml.
...
* `cli/forward.py` -- Add a `--log-format` option (default: `text`, also supports `jsonl`) following the same pattern as `mng`'s logging options. When `jsonl`, structured events (including the login URL) are emitted to stderr as parseable JSONL lines. Electron launches with `--log-format jsonl` to reliably parse events.
