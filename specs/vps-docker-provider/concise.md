...
* `mngr_vps_docker` provides two base provider classes for running Docker containers on VPS instances, differing in what they treat as "the host":
...
* `mngr create --provider vultr-always-on` provisions a Vultr VPS, installs Docker via SSH, runs a container with sshd, and returns an online host accessible via `ssh -J user@vps root@localhost:2222`
* `mngr create --provider vultr-ephemeral` does the same, but also attaches a Vultr Block Storage volume and mounts it as the Docker named volume's backing store
* `mngr stop` on an always-on host does `docker stop` on the remote container; the VPS keeps running. `mngr start` does `docker start`.
* `mngr stop` on an ephemeral host halts the Vultr VPS instance (which preserves disk state). `mngr start` starts the VPS and then starts the Docker container.
* `mngr destroy` on either variant destroys the Docker container, the VPS instance, and any associated block storage volumes
* `mngr ls` works for both online and offline hosts. For always-on hosts, offline state data is read via SSH to the VPS + `docker exec` into a state container (same pattern as existing Docker provider). For ephemeral hosts, offline discovery relies on locally cached certified data (since the VPS itself is down).
* `mngr snapshot create` uses `docker commit` for always-on hosts and the Vultr snapshot API for ephemeral hosts
* `mngr ssh` connects through the VPS as a jump host to the container
...
* Both packages register via pluggy entry points (same pattern as `mngr_modal`)
