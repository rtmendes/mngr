...
When adding a new common ratchet, add the test function to `imbue_common`'s `test_ratchets.py` (the canonical source of truth), then run `uv run python scripts/sync_common_ratchets.py` to propagate it to all other projects. Finally, run `uv run pytest --inline-snapshot=update -k test_ratchets` to set the actual violation counts per project. See the `/writing-ratchet-tests` skill for the full workflow.
