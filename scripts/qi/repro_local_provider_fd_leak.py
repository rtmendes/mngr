"""Repro script: exercise only the local provider and monitor FD count.

Tests whether list_agents leaks FDs when only the local provider is active
(no Modal, no SSH).

Usage:
    uv run python scripts/qi/repro_local_provider_fd_leak.py [--iterations N]
"""
