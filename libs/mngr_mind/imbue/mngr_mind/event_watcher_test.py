...
from imbue.mngr_llm.conftest import write_minds_settings_toml
from imbue.mngr_mind import event_watcher as event_watcher_module
from imbue.mngr_mind.conftest import EventWatcherSubprocessCapture
from imbue.mngr_mind.conftest import FakeWaitProcess
from imbue.mngr_mind.conftest import SyntheticLoopEnv
from imbue.mngr_mind.conftest import TrackingIdleWait
from imbue.mngr_mind.conftest import _create_fake_wait_process
from imbue.mngr_mind.conftest import _create_synthetic_loop_env
from imbue.mngr_mind.conftest import create_executable_command
...
from imbue.mngr_mind.event_watcher import _DEFAULT_MAX_DELIVERY_RETRIES
from imbue.mngr_mind.event_watcher import _DEFAULT_MAX_EVENT_LENGTH
from imbue.mngr_mind.event_watcher import _DEFAULT_MAX_MESSAGES_PER_MINUTE
from imbue.mngr_mind.event_watcher import _DEFAULT_MAX_SAME_SOURCE_EVENTS_PER_BATCH
from imbue.mngr_mind.event_watcher import _DeliveryState
from imbue.mngr_mind.event_watcher import _EventWatcherSettings
from imbue.mngr_mind.event_watcher import _IgnoredSourcesState
from imbue.mngr_mind.event_watcher import _ONBOARDING_MARKER_FILENAME
from imbue.mngr_mind.event_watcher import _SCHEDULED_STATE_FILENAME
from imbue.mngr_mind.event_watcher import _SendRateTracker
from imbue.mngr_mind.event_watcher import _TokenBucket
from imbue.mngr_mind.event_watcher import _apply_event_batch_filter
from imbue.mngr_mind.event_watcher import _apply_special_event_handling
from imbue.mngr_mind.event_watcher import _compute_backoff_seconds
from imbue.mngr_mind.event_watcher import _deliver_batch
from imbue.mngr_mind.event_watcher import _filter_catchup_events
from imbue.mngr_mind.event_watcher import _filter_ignored_sources
from imbue.mngr_mind.event_watcher import _get_system_notifications_conversation_id
from imbue.mngr_mind.event_watcher import _load_delivery_state
from imbue.mngr_mind.event_watcher import _load_ignored_sources_if_updated
from imbue.mngr_mind.event_watcher import _load_scheduled_events_state
from imbue.mngr_mind.event_watcher import _load_watcher_settings
from imbue.mngr_mind.event_watcher import _make_synthetic_event_line
from imbue.mngr_mind.event_watcher import _parse_time_of_day
from imbue.mngr_mind.event_watcher import _per_event_idle_delay_minutes
from imbue.mngr_mind.event_watcher import _resolve_user_timezone
from imbue.mngr_mind.event_watcher import _run_delivery_loop
from imbue.mngr_mind.event_watcher import _run_event_batch_filter_command
from imbue.mngr_mind.event_watcher import _run_synthetic_events_loop
from imbue.mngr_mind.event_watcher import _save_delivery_state
from imbue.mngr_mind.event_watcher import _save_scheduled_events_state
from imbue.mngr_mind.event_watcher import _send_chat_notification
from imbue.mngr_mind.event_watcher import _send_message
from imbue.mngr_mind.event_watcher import _separate_chat_events
from imbue.mngr_mind.event_watcher import _should_skip_for_catchup
from imbue.mngr_mind.event_watcher import _write_events_file
from imbue.mngr_mind.event_watcher import _write_notification_event
...
def test_send_message_returns_true_on_success(mock_subprocess_success: EventWatcherSubprocessCapture) -> None:
    ...
    assert any("mngr" in c for c in cmd)
...
def test_deliver_batch_updates_state_on_success(
    ...
    # Verify mngr message was called with a file path reference
    assert len(mock_subprocess_success.calls) == 1
    cmd = mock_subprocess_success.calls[0][0]
...
def test_apply_special_event_handling_aggregates_when_event_exceeds_length(tmp_path: Path) -> None:
    ...
    short_event = json.dumps({"event_id": "evt-1", "timestamp": "2026-03-01T12:00:00Z", "source": "mngr/agents"})
    long_event = json.dumps(
        {"event_id": "evt-2", "timestamp": "2026-03-01T12:01:00Z", "source": "mngr/agents", "data": "x" * 1000}
    )
    ...
    aggregate = json.loads(result[0])
    assert aggregate["type"] == "aggregate"
    assert aggregate["source"] == "mngr/agents"

    messages_event = json.loads(result[1])
    assert messages_event["event_id"] == "evt-3"

    # Aggregate file should contain both mngr/agents events
    aggregate_file = Path(aggregate["aggregate_events"][0])
    file_lines = aggregate_file.read_text().strip().split("\n")
...
def _setup_main_env(
    ...
    *,
    ...
    monkeypatch.setenv("MNGR_AGENT_STATE_DIR", str(agent_state_dir))
    monkeypatch.setenv("MNGR_AGENT_WORK_DIR", str(work_dir))
    monkeypatch.setenv("MNGR_AGENT_ID", "agent-test-00000000000000000001")
...
def test_filter_ignored_sources_removes_matching() -> None:
    lines = [
        ...
        json.dumps({"source": "mngr/agents", "event_id": "e3"}),
    ]
...
def test_load_ignored_sources_rereads_on_mtime_change(tmp_path: Path) -> None:
    ...
    assert updated_result == frozenset({"messages", "scheduled", "mngr/agents"})
...
def test_idle_wait_ignores_real_events_when_not_idle(synthetic_loop_env: SyntheticLoopEnv) -> None:
    """Real events are ignored by the idle tracker when the agent is not idle."""
    env = synthetic_loop_env
    env.last_real_event_monotonic[0] = 1000.0
    settings = _EventWatcherSettings(idle_event_delay_minutes_schedule=(1,))
    ...
    # Agent never entered WAITING, so only the initial wait process was started
    assert len(wait_calls) == 1
    idle_events = [line for line in env.event_buffer if '"mind/idle"' in line]
    assert len(idle_events) == 0


def test_idle_wait_restarts_after_delay(synthetic_loop_env: SyntheticLoopEnv) -> None:
    """After idle state is cleared by a real event, the wait restarts after the delay."""
    env = synthetic_loop_env
    env.last_real_event_monotonic[0] = 1000.0
    settings = _EventWatcherSettings(idle_event_delay_minutes_schedule=(1,))
    ...
    def clock() -> float:
        ...
        if call_count == 1:
            ...
            return 1050.0
        if call_count == 2:
            # Real event arrives while idle -- schedules restart
            env.last_real_event_monotonic[0] = 1060.0
            return 1060.0
        if call_count == 3:
            # Not enough delay yet (restart_at = 1060 + 15 = 1075)
            return 1070.0
        if call_count == 4:
            # Enough delay (T=1076 >= restart_at=1075)
            return 1076.0
        if call_count > 5:
            env.stop_event.set()
        return 1100.0
    ...
    # Initial wait + restart after delay = 2 processes
    assert len(tracker.processes) == 2
    # No idle events (second wait process is still pending)
    idle_events = [line for line in env.event_buffer if '"mind/idle"' in line]
    assert len(idle_events) == 0
...
def test_idle_wait_restarts_on_nonzero_exit(synthetic_loop_env: SyntheticLoopEnv) -> None:
    """When mngr wait exits with a non-zero code, it is restarted."""
    env = synthetic_loop_env
    env.last_real_event_monotonic[0] = 1000.0
    settings = _EventWatcherSettings(idle_event_delay_minutes_schedule=(1,))
...
def _make_first_idle_cycle_clock(
    ...
    """Build a clock that fires the first idle event, simulates wake-up, then delegates.
    ...
    Implements the common 5-step idle cycle used by multiple tests:
      ...
      3. Conversation events arrive (messages source), no effect on idle tracking
      4. Restart delay elapsed (T=1137 > restart_at=1135), new wait process starts
...
def test_idle_wait_preserves_counter_across_messages_events(synthetic_loop_env: SyntheticLoopEnv) -> None:
    ...
    clock = _make_first_idle_cycle_clock(env, tracker, post_reentry)
    idle_events = _run_idle_cycle_loop(env, settings, tracker, clock)
...
def test_idle_wait_resets_counter_on_non_messages_event(synthetic_loop_env: SyntheticLoopEnv) -> None:
    ...
    Events from sources other than "messages" (e.g. mngr/agents, monitor)
    ...
    def post_reentry(n: int) -> float | None:
        if n == 6:
            # A genuinely new non-messages event arrives while idle -- resets counter
            env.last_real_event_monotonic[0] = 1200.0
            env.last_non_messages_event_monotonic[0] = 1200.0
            return 1200.0
        if n == 7:
            # restart_at = 1200 + 15 = 1215. T=1220 >= 1215 -> restart in tick()
            return 1220.0
        if n == 8:
            # 3rd process was created by tick() at step 7. Complete it now.
            if len(tracker.processes) >= 3:
                tracker.processes[-1].complete(0)
            return 1225.0
        if n == 9:
            # (1295 - 1225) / 60 = 1.17 > 1 -> idle event fires (counter was reset)
            return 1295.0
        if n > 10:
            return None
        return 1400.0
