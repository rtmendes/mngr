...
# timeout-path invokes `process.terminate(force_kill_seconds=0.0)` at
# concurrency_group.py:228 (fire-and-forget), so the child process is not
# guaranteed to be reaped before pytest's leak detector runs. Retry is safe
# because the assertion contents (StrandTimedOutError + ProcessError +
