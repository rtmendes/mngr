...
def _get_zygote_source_dir() -> Path:
...
def test_prevent_todos() -> None:
    chunks = check_ratchet_rule(PREVENT_TODOS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_TODOS.format_failure(chunks)


def test_prevent_exec_usage() -> None:
    chunks = check_ratchet_rule(PREVENT_EXEC, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(5), PREVENT_EXEC.format_failure(chunks)


def test_prevent_eval_usage() -> None:
    chunks = check_ratchet_rule(PREVENT_EVAL, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_EVAL.format_failure(chunks)


def test_prevent_while_true() -> None:
    chunks = check_ratchet_rule(PREVENT_WHILE_TRUE, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(3), PREVENT_WHILE_TRUE.format_failure(chunks)


def test_prevent_time_sleep() -> None:
    chunks = check_ratchet_rule(PREVENT_TIME_SLEEP, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(1), PREVENT_TIME_SLEEP.format_failure(chunks)


def test_prevent_global_keyword() -> None:
    chunks = check_ratchet_rule(PREVENT_GLOBAL_KEYWORD, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_GLOBAL_KEYWORD.format_failure(chunks)


def test_prevent_bare_print() -> None:
    chunks = check_ratchet_rule(PREVENT_BARE_PRINT, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_bare_except() -> None:
    chunks = check_ratchet_rule(PREVENT_BARE_EXCEPT, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_BARE_EXCEPT.format_failure(chunks)


def test_prevent_broad_exception_catch() -> None:
    chunks = check_ratchet_rule(PREVENT_BROAD_EXCEPTION_CATCH, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(8), PREVENT_BROAD_EXCEPTION_CATCH.format_failure(chunks)


def test_prevent_base_exception_catch() -> None:
    chunks = check_ratchet_rule(PREVENT_BASE_EXCEPTION_CATCH, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_BASE_EXCEPTION_CATCH.format_failure(chunks)


def test_prevent_builtin_exception_raises() -> None:
    chunks = check_ratchet_rule(PREVENT_BUILTIN_EXCEPTION_RAISES, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_inline_imports() -> None:
    chunks = check_ratchet_rule(PREVENT_INLINE_IMPORTS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(30), PREVENT_INLINE_IMPORTS.format_failure(chunks)


def test_prevent_relative_imports() -> None:
    chunks = check_ratchet_rule(PREVENT_RELATIVE_IMPORTS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_RELATIVE_IMPORTS.format_failure(chunks)


def test_prevent_import_datetime() -> None:
    chunks = check_ratchet_rule(PREVENT_IMPORT_DATETIME, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_IMPORT_DATETIME.format_failure(chunks)


def test_prevent_importlib_import_module() -> None:
    chunks = check_ratchet_rule(PREVENT_IMPORTLIB_IMPORT_MODULE, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_IMPORTLIB_IMPORT_MODULE.format_failure(chunks)


def test_prevent_getattr() -> None:
    chunks = check_ratchet_rule(PREVENT_GETATTR, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_GETATTR.format_failure(chunks)


def test_prevent_setattr() -> None:
    chunks = check_ratchet_rule(PREVENT_SETATTR, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_pandas_import() -> None:
    chunks = check_ratchet_rule(PREVENT_PANDAS_IMPORT, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_PANDAS_IMPORT.format_failure(chunks)


def test_prevent_dataclasses_import() -> None:
    chunks = check_ratchet_rule(PREVENT_DATACLASSES_IMPORT, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(1), PREVENT_DATACLASSES_IMPORT.format_failure(chunks)


def test_prevent_namedtuple_usage() -> None:
    chunks = check_ratchet_rule(PREVENT_NAMEDTUPLE, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_NAMEDTUPLE.format_failure(chunks)


def test_prevent_yaml_usage() -> None:
    chunks = check_ratchet_rule(PREVENT_YAML_USAGE, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_YAML_USAGE.format_failure(chunks)


def test_prevent_functools_partial() -> None:
    chunks = check_ratchet_rule(PREVENT_FUNCTOOLS_PARTIAL, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_num_prefix() -> None:
    chunks = check_ratchet_rule(PREVENT_NUM_PREFIX, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_trailing_comments() -> None:
    chunks = check_ratchet_rule(PREVENT_TRAILING_COMMENTS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(38), PREVENT_TRAILING_COMMENTS.format_failure(chunks)


def test_prevent_init_docstrings() -> None:
    chunks = check_ratchet_rule(PREVENT_INIT_DOCSTRINGS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_INIT_DOCSTRINGS.format_failure(chunks)


@pytest.mark.timeout(10)
def test_prevent_args_in_docstrings() -> None:
    chunks = check_ratchet_rule(PREVENT_ARGS_IN_DOCSTRINGS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_ARGS_IN_DOCSTRINGS.format_failure(chunks)


@pytest.mark.timeout(10)
def test_prevent_returns_in_docstrings() -> None:
    chunks = check_ratchet_rule(PREVENT_RETURNS_IN_DOCSTRINGS, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_literal_with_multiple_options() -> None:
    chunks = check_ratchet_rule(PREVENT_LITERAL_MULTIPLE_OPTIONS, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_bare_generic_types() -> None:
    chunks = check_ratchet_rule(PREVENT_BARE_GENERIC_TYPES, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_typing_builtin_imports() -> None:
    chunks = check_ratchet_rule(PREVENT_TYPING_BUILTIN_IMPORTS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_TYPING_BUILTIN_IMPORTS.format_failure(chunks)


def test_prevent_short_uuid_ids() -> None:
    chunks = check_ratchet_rule(PREVENT_SHORT_UUID_IDS, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_model_copy() -> None:
    chunks = check_ratchet_rule(PREVENT_MODEL_COPY, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_fstring_logging() -> None:
    chunks = check_ratchet_rule(PREVENT_FSTRING_LOGGING, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_FSTRING_LOGGING.format_failure(chunks)


def test_prevent_click_echo() -> None:
    chunks = check_ratchet_rule(PREVENT_CLICK_ECHO, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_unittest_mock_imports() -> None:
    chunks = check_ratchet_rule(PREVENT_UNITTEST_MOCK_IMPORTS, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_UNITTEST_MOCK_IMPORTS.format_failure(chunks)


def test_prevent_monkeypatch_setattr() -> None:
    chunks = check_ratchet_rule(PREVENT_MONKEYPATCH_SETATTR, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(6), PREVENT_MONKEYPATCH_SETATTR.format_failure(chunks)


def test_prevent_test_container_classes() -> None:
    all_chunks = check_ratchet_rule(PREVENT_TEST_CONTAINER_CLASSES, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_pytest_mark_integration() -> None:
    chunks = check_ratchet_rule(PREVENT_PYTEST_MARK_INTEGRATION, _get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_os_fork() -> None:
    chunks = check_ratchet_rule(PREVENT_OS_FORK, _get_zygote_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(0), PREVENT_OS_FORK.format_failure(chunks)


def test_prevent_direct_subprocess_usage() -> None:
    """Prevent direct usage of subprocess in non-test code.

    Test files and the standalone resource scripts (event_watcher.py, etc.)
    are excluded since they run outside the ConcurrencyGroup framework.
    """
...
def test_prevent_if_elif_without_else() -> None:
    chunks = find_if_elif_without_else(_get_zygote_source_dir(), _SELF_EXCLUSION)
...
def test_prevent_inline_functions_in_non_test_code() -> None:
    chunks = find_inline_functions(_get_zygote_source_dir())
    assert len(chunks) <= snapshot(6), PREVENT_INLINE_FUNCTIONS.format_failure(chunks)


def test_prevent_importing_underscore_prefixed_names_in_non_test_code() -> None:
    chunks = find_underscore_imports(_get_zygote_source_dir())
    assert len(chunks) <= snapshot(0), PREVENT_UNDERSCORE_IMPORTS.format_failure(chunks)


def test_prevent_init_methods_in_non_exception_classes() -> None:
    chunks = find_init_methods_in_non_exception_classes(_get_zygote_source_dir())
    assert len(chunks) <= snapshot(9), PREVENT_INIT_IN_NON_EXCEPTION_CLASSES.format_failure(chunks)


def test_prevent_cast_usage() -> None:
    chunks = find_cast_usages(_get_zygote_source_dir())
    assert len(chunks) <= snapshot(0), PREVENT_CAST_USAGE.format_failure(chunks)


def test_prevent_assert_isinstance_usage() -> None:
    chunks = find_assert_isinstance_usages(_get_zygote_source_dir())
...
def test_prevent_code_in_init_files() -> None:
    ...
    violations = find_code_in_init_files(_get_zygote_source_dir())
