...
def set_uv_tool_env_vars(env_vars: dict[str, str]) -> None:
    """Set UV_TOOL_DIR and UV_TOOL_BIN_DIR from MNGR_AGENT_STATE_DIR.

    Ensures that ``uv tool install`` (run by the mngr_recursive plugin)
    ...
    ClaudeMindAgent.
    """
    agent_state_dir = env_vars.get("MNGR_AGENT_STATE_DIR", "")
    if agent_state_dir:
...
def set_llm_model_env_var(
    ...
    """Set MNGR_LLM_MODEL from loaded settings or hardcoded default.
...
class LlmAgent(BaseAgent[LlmAgentConfig]):
    ...
    def assemble_command(
        ...
        parts = ["$MNGR_AGENT_STATE_DIR/commands/chat.sh", "--new", "--name", '"Inner Monologue"']
    ...
    def provision(
        ...
        Steps:
        1. Per-agent mngr installation (via mngr_recursive)
...
def register_cli_commands() -> Sequence[click.Command] | None:
    """Register llm supporting service commands with mngr."""
    from imbue.mngr_llm.cli import get_all_commands
