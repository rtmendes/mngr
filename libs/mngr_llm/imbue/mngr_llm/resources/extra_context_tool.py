...
Settings are read from $MNGR_AGENT_WORK_DIR/minds.toml.
...
where they cannot import from each other or from the mngr_claude_mind package.
"""
...
def _get_mngr_command() -> list[str]:
    """Return the command for invoking the per-agent mngr binary.

    Looks for the mngr binary in ``$UV_TOOL_BIN_DIR/mngr``. Raises RuntimeError
    if the binary cannot be found.

    NOTE: This is a standalone copy of watcher_common.get_mngr_command()
    ...
    if not bin_dir:
        raise RuntimeError("UV_TOOL_BIN_DIR is not set. The per-agent mngr binary cannot be located without it.")
...
_MNGR_LIST_HARD_TIMEOUT = _EXTRA.get("mngr_list_hard_timeout_seconds", 120)
_MNGR_LIST_WARN_THRESHOLD = _EXTRA.get("mngr_list_warn_threshold_seconds", 15)
...
def gather_extra_context() -> str:
    ...
    - Current mngr agent list (active agents and their states)
    ...
    # Current mngr agent list
    try:
        start = time.monotonic()
        result = subprocess.run(
            [*_get_mngr_command(), "list", "--format", "json"],
            capture_output=True,
            text=True,
            timeout=_MNGR_LIST_HARD_TIMEOUT,
        )
        elapsed = time.monotonic() - start
        if elapsed > _MNGR_LIST_WARN_THRESHOLD:
            print(
                f"WARNING: mngr list took {elapsed:.1f}s (expected <{_MNGR_LIST_WARN_THRESHOLD}s)",
                file=sys.stderr,
            )
        ...
        sections.append(f"## Current Agents\n(Timed out after {_MNGR_LIST_HARD_TIMEOUT}s -- mngr list may be hanging)")
    except (FileNotFoundError, OSError, RuntimeError):
        sections.append("## Current Agents\n(Unable to retrieve agent list)")

    agent_data_dir_str = os.environ.get("MNGR_AGENT_STATE_DIR", "")
