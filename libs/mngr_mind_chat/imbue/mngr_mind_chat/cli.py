...
from imbue.mngr_mind_chat.api import ConversationInfo
...
from imbue.mngr_mind_chat.api import run_chat_on_agent
...
def chat(ctx: click.Context, **kwargs: Any) -> None:  # pragma: no cover
    ...
    chat_args = resolve_chat_args(opts, agent, host, is_interactive=mngr_ctx.is_interactive)
...
CommandHelpMetadata(
    ...
    synopsis="mngr chat [OPTIONS] [AGENT]",
...
The agent can be specified as a positional argument or via --agent:
  ...
  mngr chat --agent my-agent""",
    examples=(
        ("Start a new named conversation", 'mngr chat my-agent --new --name "Bug triage"'),
        ("Resume the most recent conversation", "mngr chat my-agent --last"),
        ("Resume a specific conversation", "mngr chat my-agent --conversation conv-1234567890-abcdef"),
        ("Show interactive agent selector", "mngr chat"),
        ("Show interactive conversation selector", "mngr chat my-agent"),
    ),
    see_also=(
