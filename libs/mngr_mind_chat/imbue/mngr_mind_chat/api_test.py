"""Unit tests for the mngr-mind-chat API module."""
...
from imbue.mngr_mind_chat.api import ChatCommandError
from imbue.mngr_mind_chat.api import _build_chat_env_vars
from imbue.mngr_mind_chat.api import _build_conversation_db_and_messages_paths
from imbue.mngr_mind_chat.api import _build_remote_chat_script
from imbue.mngr_mind_chat.api import _load_env_file_into_dict
from imbue.mngr_mind_chat.api import get_latest_conversation_id
from imbue.mngr_mind_chat.api import list_conversations_on_agent
from imbue.mngr_mind_chat.testing import TestAgent
from imbue.mngr_mind_chat.testing import create_conversation_events
from imbue.mngr_mind_chat.testing import create_message_events
...
def test_build_chat_env_vars_contains_all_required_keys(
    ...
    assert env_vars["MNGR_HOST_DIR"] == str(host.host_dir)
    assert env_vars["MNGR_AGENT_STATE_DIR"] == str(host.host_dir / "agents" / str(agent.id))
    assert env_vars["MNGR_AGENT_WORK_DIR"] == str(agent.work_dir)
    assert env_vars["MNGR_AGENT_ID"] == str(agent.id)
    assert env_vars["MNGR_AGENT_NAME"] == str(agent.name)
...
def test_build_remote_chat_script_sources_env_files(
    ...
    # Should source env files before setting MNGR_ vars
