...
CommandHelpMetadata(
    ...
    synopsis="mng file (get|put|list) TARGET PATH [OPTIONS]",
...
relative paths always resolve against the host directory.""",
    examples=(
        ("Read a file from an agent", "mng file get my-agent config.toml"),
        ("Write a file to an agent", "mng file put my-agent config.toml --input local.toml"),
        ("List files in an agent's work directory", "mng file list my-agent"),
        ("List files relative to agent state directory", "mng file list my-agent --relative-to state"),
        ("Read a file using absolute path", "mng file get my-agent /etc/hostname"),
        ("Write stdin to a file on a host", "echo 'hello' | mng file put my-host greeting.txt"),
    ),
    see_also=(
