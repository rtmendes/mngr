...
from imbue.mngr_llm.provisioning import execute_with_timing
...
# and does not belong in the generic mngr_mind plugin.
...
#
# Reads all event IDs from $MNGR_AGENT_STATE_DIR/mind/event_batches/*.jsonl,
# compares them against handled event IDs extracted from
# $MNGR_AGENT_STATE_DIR/events/handled_events/events.jsonl (and .jsonl.1),
# and exits with code 2 if any are unhandled (which tells Claude Code to
# block the stop).

set -euo pipefail

batches_dir="$MNGR_AGENT_STATE_DIR/mind/event_batches"
handled_dir="$MNGR_AGENT_STATE_DIR/events/handled_events"
