...
Permissions are **opaque strings**. mng itself knows nothing about their content or format -- it simply stores them as a list and passes them along to plugins. All interpretation of what a permission means is done by plugins.
...
- **Revoking a permission requires an exact string match.** When you use `--revoke`, mng compares strings exactly. If you granted `"github:read"`, you must revoke exactly `"github:read"` -- not `"github:READ"` or `"Github:read"`.
- **mng does not validate permissions.** It will accept any string you grant. Whether a permission actually does anything depends on whether a plugin recognizes it.
