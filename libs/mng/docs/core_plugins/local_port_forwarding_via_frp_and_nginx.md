...
```
<service>.<agent>.<host>.mng.localhost:8080
```

For example:
- `api.alice.dev-box.mng.localhost:8080` - An API exposed by agent "alice"
- `web.bob.dev-box.mng.localhost:8080` - A web app from agent "bob" on the same host
...
Run `mng auth` [future] to set an authentication cookie in your browser. After this, all `*.mng.localhost` URLs will work automatically.

### Programmatic Access

For scripts and tools, use the `X-Mng-Auth` header:

```bash
curl -H "X-Mng-Auth: $(cat ~/.config/mng/auth_token)" \
  http://api.alice.dev-box.mng.localhost:8080/
```
