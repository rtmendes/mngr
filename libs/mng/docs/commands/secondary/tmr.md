...
```text
mng tmr [TEST_PATHS...] [-- TESTING_FLAGS...] [--provider <PROVIDER>] [--use-snapshot] [--env KEY=VALUE] [--label KEY=VALUE] [--timeout <SECS>] [--agent-type <TYPE>]
```
...
```bash
$ mng tmr --provider docker tests/
```

**Modal with snapshot**

```bash
$ mng tmr --provider modal --use-snapshot tests/
```
...
```bash
$ mng tmr --poll-interval 30
```

**Specify output location**

```bash
$ mng tmr --output-html report.html
```
