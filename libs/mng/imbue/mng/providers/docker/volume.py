...
STATE_VOLUME_MOUNT_PATH: Final[str] = "/mng-state"
...
class DockerVolume(BaseVolume):
    ...
    def remove_file(self, path: str, *, recursive: bool = False) -> None:
        ...
        if exit_code != 0:
            raise MngError(f"Failed to remove '{path}' from volume: {output}")
    ...
    def remove_directory(self, path: str) -> None:
        ...
        if exit_code != 0:
            raise MngError(f"Failed to remove directory '{path}' from volume: {output}")
    ...
    def write_files(self, file_contents_by_path: Mapping[str, bytes]) -> None:
        ...
        if not success:
            raise MngError("Failed to write files to Docker volume")
