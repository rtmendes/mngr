...
def test_host_volume_get_agent_volume_write_goes_to_correct_path() -> None:
    ...
    host_volume = HostVolume(volume=vol)
    agent_volume = host_volume.get_agent_volume(agent_id)
    agent_volume.write_files({"events/claude_transcript/events.jsonl": b"line1\nline2\n"})
    assert vol.files[f"agents/{agent_id}/events/claude_transcript/events.jsonl"] == b"line1\nline2\n"
