...
Per-host ssh keys are stored in `~/.ssh/mng/$HOST_ID` and deleted when the host is destroyed. If no key exists, `mng` assumes access is configured via `~/.ssh/config` or agent forwarding.

## Heartbeat Mechanism

Remote environments need a heartbeat script baked into the container to detect when the controlling `mng` process dies unexpectedly.

Without this mechanism, the following failure scenario could occur:
1. `mng` starts a remote host
2. The host starts successfully and agents begin running
3. The outer `mng` process crashes or the network connection is lost
