...
Currently, users must know plugin names to install them. A `mng plugin search` command could:
...
Currently, there's no direct affordance for agent-to-agent communication beyond `mng message`. Future options:
...
See [specs/plugins/recursive_mng.md](./plugins/recursive_mng.md) for details on the security model for recursive agent creation.
