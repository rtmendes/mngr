...
# FIXME: pending slack discussion
...
# FIXME: pending slack discussion. Also this section (and command) needs some work -- idk about the "--", also maybe send via stdin, also some garbage options to remove (timeout, user)
...
mng exec my-task --user root -- apt-get update
...
# FIXME: same questions about selection, --all, etc
# auto-generated by Claude, remove when a human has sanctioned this
...
# TODO: there are some other behaviors with starting from other snapshots that we should either document or remove
#  I'm leaning towards remove, since it doesn't *quite* make sense to start from a non-latest snapshot--that's about the host,
#  and if there were multiple agents on the same host, it's a bit confusing

# stop a running agent
mng stop my-task

# FIXME: pending question
# auto-generated by Claude, remove when a human has sanctioned this
# short form
mng s my-task
...
# FIXME: same questions about filtering
# auto-generated by Claude, remove when a human has sanctioned this
...
# FIXME: hmmm, I think we could remove almost all of the options and complexity from gc...
...
# FIXME: silly, we should remove these and make the examples just about using tail and head
...
# FIXME: these commands need to change probably
...
mng exec my-task -- bash -c 'git add . && git commit -am "Please commit all your changes with a descriptive message"'
...
mng list --include 'labels.team == "backend"'
...
mng destroy --include 'labels.team == "backend"' --include 'state == "STOPPED"' --force --dry-run
...
mng destroy -f --include 'host.provider == "docker"'
...
mng snapshot create batch-job --name "after-training"
...
mng list --running --format json | python -c "
import sys, json
