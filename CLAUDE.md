...
- To run tests for a single project, use a subshell so you don't change your working directory: "(cd libs/mngr && uv run pytest)" or "(cd apps/minds && uv run pytest)". Each project has its own pytest and coverage configuration in its pyproject.toml. IMPORTANT: always wrap the cd in parentheses `(...)` to avoid changing your shell's working directory.
