...
class SharingStatus(FrozenModel):
    """Forwarding status for a service."""

    enabled: bool = Field(description="Whether Cloudflare forwarding is active for this service")
...
def _cloudflare_url(service_name: str) -> str:
    """Build the desktop client API URL for a service's Cloudflare forwarding."""
...
def get_sharing_status(service_name: str) -> SharingStatus:
    """Fetch the current Cloudflare forwarding status for a service.
