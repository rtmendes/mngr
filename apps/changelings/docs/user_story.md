...
4. We prepare a temporary repo (either by cloning or by creating an empty git repo with .mng/settings.toml), generate an agent ID, and move the repo to `~/.changelings/<agent-id>/`. For local deployment, we run `mng create --in-place --id <id> -t entrypoint --label changeling=true` from that directory. For remote deployment, we run `mng create --in <provider> --id <id> --source-path <dir> -t entrypoint --label changeling=true` and clean up the local directory afterwards.
