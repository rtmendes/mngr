...
1. User runs: `changeling deploy --type elena-code` (or `changeling deploy <git-url>` for an existing repo with agent_type in changelings.toml)
...
4. We prepare a temporary repo (either by cloning or by creating an empty git repo), resolve the agent type (from --type or changelings.toml), generate an agent ID, and move the repo to `~/.changelings/<agent-id>/`. For local deployment, we run `mng create --in-place --id <id> --type <type> --label changeling=true` from that directory. For remote deployment, we run `mng create --in <provider> --id <id> --type <type> --source-path <dir> --label changeling=true` and clean up the local directory afterwards.
