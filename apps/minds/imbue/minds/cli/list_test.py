...
from imbue.minds.cli.list import _DEFAULT_DISPLAY_FIELDS
from imbue.minds.cli.list import _HEADER_LABELS
from imbue.minds.cli.list import _build_table
from imbue.minds.cli.list import _emit_human_output
from imbue.minds.cli.list import _emit_json_output
from imbue.minds.cli.list import _get_field_value
...
def test_emit_human_output_with_empty_list_shows_no_minds() -> None:
    """Verify that _emit_human_output shows 'No minds found' for empty list."""
    with capture_loguru_messages() as messages:
        _emit_human_output([], _DEFAULT_DISPLAY_FIELDS)

    combined = "".join(messages)
    assert "No minds found" in combined
...
def test_emit_json_output_produces_valid_json(capsys: Any) -> None:
    ...
    assert "minds" in parsed
    assert len(parsed["minds"]) == 1
    assert parsed["minds"][0]["name"] == "my-bot"


def test_emit_json_output_produces_empty_list_for_no_agents(capsys: Any) -> None:
    """Verify that _emit_json_output produces an empty minds list."""
    _emit_json_output([])
...
def test_list_help() -> None:
    """Verify that mind list --help works."""
    result = _RUNNER.invoke(cli, ["list", "--help"])

    assert result.exit_code == 0
    assert "List deployed minds" in result.output
