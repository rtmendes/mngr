...
from imbue.minds.deployment.local import UpdateResult
...
def _is_agent_remote(
    ...
    if concurrency_group is not None:
        ...
        cg = ConcurrencyGroup(name="mind-check-remote")
...
def _run_update(
    ...
    Raises MindError if any step fails.
    """
    cg = ConcurrencyGroup(name="mind-update")
    update_error: MindError | None = None
    with cg:
        try:
            result = update_local(
...
def _print_result(result: UpdateResult) -> None:
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("Mind updated successfully")
    logger.info("=" * 60)
    logger.info("")
...
def update(
    ...
    """Update an existing mind by redeploying into it.

    AGENT_NAME is the name of the mind to update.
    ...
    \b
        mind update my-agent
        mind update my-agent --no-snapshot
        mind update my-agent --no-push --no-provision
    """
    parsed_name = AgentName(agent_name)

    if _is_agent_remote(parsed_name):
        raise MindError(
            "Updating remote minds is not yet supported. Remote update requires mng push/pull to support remote hosts."
        )

    logger.info("Updating mind '{}'...", agent_name)
