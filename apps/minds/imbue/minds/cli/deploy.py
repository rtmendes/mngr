...
from imbue.minds.deployment.local import clone_git_repo
from imbue.minds.deployment.local import commit_files_in_repo
from imbue.minds.deployment.local import deploy_mind
from imbue.minds.deployment.local import init_empty_git_repo
...
def _run_deployment(
    ...
    """Deploy the mind and return the result.
    ...
    cg = ConcurrencyGroup(name="mind-deploy")
    deploy_error: MindError | None = None
    with cg:
        try:
            result = deploy_mind(
                mind_dir=mind_dir,
...
def _print_result(result: DeploymentResult, provider: DeploymentProvider) -> None:
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("Mind deployed successfully")
    logger.info("=" * 60)
    logger.info("")
    ...
    logger.info("")
    logger.info("Start the forwarding server to access your mind:")
    logger.info("  mind forward")
    logger.info("=" * 60)
...
def _prepare_repo(
    ...
    if git_url is not None:
        ...
        logger.info("Creating mind repo")
    ...
    if git_url is None or files_added > 0:
        commit_files_in_repo(temp_dir, "Initial mind setup")


def _resolve_agent_type(temp_dir: Path, cli_agent_type: str | None) -> str:
    """Resolve the agent type from CLI flag or minds.toml.

    Resolution order:
    1. CLI --agent-type flag (if provided)
    2. agent_type field in minds.toml (if present in the repo)
    3. Error

    Uses ``load_settings_from_path`` to parse minds.toml through the
    ``ClaudeMindSettings`` model, ensuring consistent validation.
    ...
    raise MissingAgentTypeError(
        ...
        "or set agent_type in minds.toml in the repository."
    )
...
def _resolve_agent_name(name: str | None, agent_type: str | None) -> AgentName:
    ...
    default_name = agent_type if agent_type is not None else "mind"
...
def _move_to_permanent_location(temp_dir: Path, mind_dir: Path) -> None:
    """Move the prepared repo from its temp location to the permanent mind directory."""
    if mind_dir.exists():
        raise MindError("A mind directory already exists at '{}'. Remove it first.".format(mind_dir))

    try:
        temp_dir.rename(mind_dir)
    except OSError as e:
        logger.debug("rename failed ({}), falling back to shutil.move", e)
        shutil.move(str(temp_dir), str(mind_dir))
...
@click.option(
    "--agent-type",
    default=None,
    help="Agent type to deploy (e.g. 'elena-code'). Required unless the repo has agent_type in minds.toml.",
)
@click.option(
    "--add-path",
    multiple=True,
...
def deploy(
    ...
    """Deploy a new mind from a git repository or agent type.

    GIT_URL is an optional git URL to clone (local path, file://, https://, or ssh).
    Alternatively, use --agent-type to create a mind without a git repository.

    Either GIT_URL or --agent-type must be provided.

    The agent type is passed directly to mng create. When --agent-type is provided,
    it takes precedence. Otherwise, the agent type is read from minds.toml
    in the repository.

    Example:

        mind deploy --agent-type elena-code

        mind deploy --agent-type elena-code --add-path ./config:config --name my-agent

        mind deploy ./my-agent-repo --agent-type elena-code

        mind deploy ./my-agent-repo
    """
    ...
    # Move the prepared repo to the permanent mind directory
    # (~/.minds/<agent-id>/) before deploying. For local, the agent
    ...
    mind_dir = paths.mind_dir(agent_id)
    try:
        _move_to_permanent_location(temp_dir, mind_dir)
    except MindError:
        shutil.rmtree(temp_dir, ignore_errors=True)
        raise

    logger.info("Deploying mind from: {}", mind_dir)
