...
def create_git_repo_with_agent_type(tmp_path: Path, agent_type: str = "elena-code") -> Path:
    """Create a minimal git repo with minds.toml specifying the agent type."""
    repo_dir = tmp_path / "my-agent-repo"
    repo_dir.mkdir()
    (repo_dir / "minds.toml").write_text('agent_type = "{}"\n'.format(agent_type))
...
def data_dir_args(tmp_path: Path) -> list[str]:
    ...
    return ["--data-dir", str(tmp_path / "minds-data")]
...
def deploy_with_agent_type(
    ...
    """Invoke mind deploy with --agent-type and standard non-interactive flags."""
...
def deploy_with_git_url(
    ...
    """Invoke mind deploy with a git URL and standard non-interactive flags."""
