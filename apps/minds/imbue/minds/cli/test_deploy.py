...
from imbue.minds.cli.conftest import deploy_with_git_url
...
def test_deploy_agent_type_shows_creating_message(tmp_path: Path) -> None:
    """Verify that --agent-type shows a 'Creating mind repo' message instead of 'Cloning'."""
    result = deploy_with_agent_type(tmp_path)

    assert "Cloning repository" not in result.output
    assert "Deploying mind from" in result.output
