...
When no vendor configuration is present, falls back to vendoring the mngr
...
MNGR_GITHUB_URL: Final[str] = "https://github.com/imbue-ai/mngr.git"

VENDOR_DIR_NAME: Final[str] = "vendor"

VENDOR_MNGR_DIR_NAME: Final[NonEmptyStr] = NonEmptyStr("mngr")
...
def default_vendor_configs() -> tuple[VendorRepoConfig, ...]:
    """Build the default vendor config (mngr repo from GitHub) when none is configured."""
    return (
        VendorRepoConfig(
            name=VENDOR_MNGR_DIR_NAME,
            url=MNGR_GITHUB_URL,
        ),
    )
