...
from imbue.minds.desktop_client.minds_config import DEFAULT_REMOTE_SERVICE_CONNECTOR_URL
...
def _clear_url_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("REMOTE_SERVICE_CONNECTOR_URL", raising=False)
...
def test_remote_service_connector_url_defaults_when_unset(tmp_path: Path) -> None:
    config = _make_config(tmp_path)
    assert config.remote_service_connector_url == AnyUrl(DEFAULT_REMOTE_SERVICE_CONNECTOR_URL)


def test_remote_service_connector_url_file_overrides_default(tmp_path: Path) -> None:
    (tmp_path / "config.toml").write_text('remote_service_connector_url = "https://cf-from-file.example.com/"\n')
    config = _make_config(tmp_path)
    assert str(config.remote_service_connector_url) == "https://cf-from-file.example.com/"


def test_remote_service_connector_url_env_overrides_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    (tmp_path / "config.toml").write_text('remote_service_connector_url = "https://cf-from-file.example.com/"\n')
    monkeypatch.setenv("REMOTE_SERVICE_CONNECTOR_URL", "https://cf-from-env.example.com/")
    config = _make_config(tmp_path)
    assert str(config.remote_service_connector_url) == "https://cf-from-env.example.com/"


def test_invalid_url_in_file_raises(tmp_path: Path) -> None:
    (tmp_path / "config.toml").write_text('remote_service_connector_url = "not-a-url"\n')
    config = _make_config(tmp_path)
    with pytest.raises(MindsConfigError):
        _ = config.remote_service_connector_url


def test_invalid_url_in_env_raises(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("REMOTE_SERVICE_CONNECTOR_URL", "not-a-url")
    config = _make_config(tmp_path)
    with pytest.raises(MindsConfigError):
        _ = config.remote_service_connector_url
...
def test_url_settings_coexist_with_user_preferences(tmp_path: Path) -> None:
    ...
    (tmp_path / "config.toml").write_text(
        'remote_service_connector_url = "https://cf.example.com/"\ndefault_account_id = "user-1"\n'
    )
    config = _make_config(tmp_path)
    assert str(config.remote_service_connector_url) == "https://cf.example.com/"
